...
function makeItemActions(typeItemActions, amendedState$) {
  return mapValues(typeItemActions, (irrelevant, actionKey) => amendedState$
      .filter(todosData => todosData.list.length)
      .flatMapLatest(todosData => Rx.Observable.merge(todosData.list.map(i => i.todoItem[actionKey]))
  )
  );
}
...
function sheet({DOM, localStorageSource}) {
